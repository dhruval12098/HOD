...
export function BlogCardDetails({
  ...
  if (simplified) {
    return (
      <div className={`${compact ? "p-4 lg:px-5 lg:py-4" : "p-5 sm:px-6 sm:py-5"} flex shrink-0 flex-col items-center text-center`}>
        <h2
          className={`${compact ? "line-clamp-2 text-[16px] leading-[1.2] lg:text-[18px]" : "line-clamp-2 text-[clamp(22px,2vw,30px)] leading-[1.2]"} blog-title-font w-full font-normal tracking-[-0.025em] text-[#0A0A0A] [&_em]:font-[inherit] [&_em]:italic`}
          dangerouslySetInnerHTML={{ __html: sanitizeEmphasisTitle(post.cardTitle || post.title) }}
        />
  ...
  return (
    <div className={`${compact ? "flex shrink-0 flex-col p-4 lg:p-5" : "flex shrink-0 flex-col p-6 sm:p-7"} items-center text-center`}>
      ...
      <h2
        className={`${compact ? "line-clamp-2 text-[16px] leading-[1.2] lg:text-[18px]" : "line-clamp-3 text-[clamp(24px,2.3vw,34px)] leading-[1.25]"} blog-title-font w-full font-semibold tracking-[-0.025em] text-[#0A1628] [&_em]:font-[inherit] [&_em]:italic`}
        dangerouslySetInnerHTML={{ __html: sanitizeEmphasisTitle(post.cardTitle || post.title) }}
      />
