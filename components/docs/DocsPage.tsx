    ...
    <main className="min-h-screen bg-(--bg) text-(--ink)">
      <section className="border-b border-black/5 bg-white/70">
        <div className="mx-auto max-w-6xl px-5 py-20 sm:px-7 lg:px-[52px]">
          ...
          <h1 className={`${cinzelFont.variable} font-primary-display mt-4 max-w-3xl text-5xl leading-none text-[#181512] sm:text-6xl`}>{title}</h1>
              ...
              {blocks.map((block, index) => (
                <section key={`${block.heading}-${index}`} className="space-y-3">
                  <div>
                    ...
                    <h3 className={`${cinzelFont.variable} font-primary-display mt-2 text-3xl leading-none text-[#181512]`}>{block.heading}</h3>
