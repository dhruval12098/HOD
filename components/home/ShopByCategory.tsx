  ...
  return <section className="bg-[var(--color-brand-tertiary,#fff)] px-[var(--space-2)] py-[var(--space-4)] sm:px-[var(--space-3)] sm:py-[var(--space-6)] xl:py-[var(--space-8)]" aria-labelledby="shop-by-category-heading">
    ...
    >
      {data.cards.map((card) => <Link key={card.id} href={card.href} className="group relative aspect-[4/5] w-[72vw] max-w-[300px] shrink-0 snap-start overflow-hidden bg-[var(--color-brand-secondary,#F9F9F9)] focus-visible:z-10 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-black">
        ...
        <span className="absolute inset-x-0 bottom-0 px-[var(--space-3)] py-[var(--space-4)] text-center font-[family-name:var(--font-family-secondary)] text-[clamp(0.58rem,0.9vw,0.72rem)] font-medium uppercase tracking-[0.09em] text-black">{card.name}</span>
      </Link>)}
    </div>
