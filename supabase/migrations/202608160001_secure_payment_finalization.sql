create or replace function public.finalize_paid_order_with_inventory_secure(
...
declare
  ...
  v_expected_amount_subunits bigint;
begin
  ...
  if nullif(trim(coalesce(v_order.razorpay_order_id, '')), '') is null
     or v_order.razorpay_order_id <> p_razorpay_order_id then
    return query select false, false, v_order.id, v_order.order_number::text, v_order.total_amount, 'payment_order_mismatch', 'Payment does not match this order.';
    return;
  end if;

  v_expected_amount_subunits := round(coalesce(v_order.payment_amount, 0) * 100)::bigint;
  if p_payment_amount_subunits is null or p_payment_amount_subunits <> v_expected_amount_subunits then
    return query select false, false, v_order.id, v_order.order_number::text, v_order.total_amount, 'payment_amount_mismatch', 'Payment amount does not match this order.';
    return;
  end if;

  if nullif(trim(coalesce(v_order.payment_currency, '')), '') is null
     or upper(v_order.payment_currency) <> upper(coalesce(p_payment_currency, '')) then
    return query select false, false, v_order.id, v_order.order_number::text, v_order.total_amount, 'payment_currency_mismatch', 'Payment currency does not match this order.';
    return;
  end if;

  if lower(coalesce(p_gateway_payment_status, '')) <> 'captured' then
    return query select false, false, v_order.id, v_order.order_number::text, v_order.total_amount, 'payment_not_captured', 'Payment has not been captured.';
    return;
  end if;

  if v_order.payment_status = 'paid' then
    if v_order.razorpay_payment_id is not null and v_order.razorpay_payment_id <> p_payment_id then
      return query select false, true, v_order.id, v_order.order_number::text, v_order.total_amount, 'payment_id_mismatch', 'Order was finalized with a different payment.';
      return;
    end if;
  ...
  v_gateway_payload :=
    ...
    || jsonb_build_object(
      ...
      coalesce(v_order.gateway_payload->'payment', '{}'::jsonb)
        || jsonb_build_object(
          'razorpayOrderId', v_order.razorpay_order_id,
          ...
          'eventType', p_gateway_payment_status,
          'amountSubunits', p_payment_amount_subunits,
          'currency', upper(p_payment_currency)
        ),
      'rawEvent',
